...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if sg1:GetCount()>0 or (sg2~=nil and sg2:GetCount()>0) then
		...
		if e:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.Equip(tp,c,tc) then
			local e1=Effect.CreateEffect(c)
...
function cm.mvtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():GetEquipTarget()~=nil
...
function cm.mvop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsAbleToDeck() and Duel.SelectYesNo(tp,aux.Stringid(m,3)) then
		Duel.BreakEffect()
