...
function c22000470.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(22000470,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_PZONE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetCountLimit(1,22000470)
	e1:SetCondition(c22000470.pspcon)
	e1:SetOperation(c22000470.pspop)
	c:RegisterEffect(e1) 
	local e2=e1:Clone()
	...
	e2:SetCondition(c22000470.pspcon2)
	c:RegisterEffect(e2)
	--tohand
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(22000470,1))
...
function c22000470.pspcon(e,tp,eg,ep,ev,re,r,rp)
...
function c22000470.pspcon2(e,tp,eg,ep,ev,re,r,rp)
	local lpz=Duel.GetFieldCard(tp,LOCATION_PZONE,0)
	local rpz=Duel.GetFieldCard(tp,LOCATION_PZONE,1)
	if rpz==nil or lpz==rpz then return false end
	local lscale=lpz:GetLeftScale()
...
function c22000470.pspop(e,tp,eg,ep,ev,re,r,rp,sg,og)
