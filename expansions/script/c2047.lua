...
function cm.initial_effect(c)
	...
	e1:SetCondition(function(e,tp) return Duel.GetAttackTarget():IsControler(tp) and Duel.GetAttackTarget():IsFaceup() and Duel.GetAttackTarget():IsSetCard(0x3905) end)
...
function cm.cecon(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	e:SetLabelObject(tc)
	return tc:IsLocation(LOCATION_MZONE) and tc:IsFaceup() and tc:IsSetCard(0x3905)
end
