...
function c40009011.initial_effect(c)
	...
	e3:SetTarget(c40009011.syntg)
	e3:SetValue(1)
	e3:SetOperation(c40009011.synop)
	c:RegisterEffect(e3)
...
function c40009011.synfilter(c,tp)
	return c:IsFacedown() and c:IsType(TYPE_MONSTER)
end
function c40009011.syntg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c40009011.synfilter(chkc,tp,c) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		and Duel.IsExistingTarget(c40009011.synfilter,tp,LOCATION_MZONE,0,1,nil,tp,c) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEDOWN)
	local g=Duel.SelectTarget(tp,c40009011.synfilter,tp,LOCATION_MZONE,0,1,1,e:GetHandler())
	Duel.SelectTarget(tp,c40009011.synfilter,tp,LOCATION_MZONE,0,1,1,nil,tp,c)
...
function c40009011.syntg(e,syncard,f,min,max)
	local minc=min+1
	local maxc=max+1
	local c=e:GetHandler()
...
function c40009011.synop(e,tp,eg,ep,ev,re,r,rp,syncard,f,min,max)
	local minc=min+1
	local maxc=max+1
	local c=e:GetHandler()
	local lv=syncard:GetLevel()
