...
function cm.initial_effect(c)
	...
	--synchro summon
	aux.AddSynchroMixProcedure(c,cm.matfilter1,nil,nil,aux.NonTuner(Card.IsSetCard,0x3ee3),1,99)
	--spsummon
...
function cm.matfilter1(c)
	return c:IsSynchroType(TYPE_TUNER) or (c:IsSynchroType(TYPE_PENDULUM) and c:IsSetCard(0x3ee3))
end
	--spsummon
...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)~=0 then
		...
		if g:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(m,7)) then
			Duel.BreakEffect()
