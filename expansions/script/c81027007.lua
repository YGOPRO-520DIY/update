...
function cm.initial_effect(c)
	...
	--Activate
	aux.AddRitualProcEqual2Code(c,81027000,nil,cm.filter)
	--destroy replace
...
function cm.repfilter(c,tp)
	return c:IsFaceup() and c:IsRace(RACE_SPELLCASTER) and c:IsLocation(LOCATION_MZONE) and c:IsType(TYPE_RITUAL) and c:IsType(TYPE_MONSTER)
