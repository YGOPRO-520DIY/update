...
function cm.initial_effect(c)
	...
	e5:SetValue(1)
	e5:SetCondition(function(e) return Duel.GetTurnPlayer()~=e:GetHandlerPlayer() and e:GetHandler():GetOverlayGroup():IsExists(Card.IsSetCard,1,nil,0x3905) end)
	c:RegisterEffect(e5)
end
...
function cm.xfilter(c,tp,xyzc)
	return c:IsSetCard(0x3905) and c:IsReleasable() and c:IsFaceup() and Duel.IsExistingMatchingCard(cm.xfilter2,tp,LOCATION_MZONE,0,1,c,tp,xyzc)
end
function cm.xfilter2(c,tp,xyzc)
	return c:IsSetCard(0x3905) and c:IsFaceup() and c:IsCanBeXyzMaterial(xyzc) and Duel.GetLocationCountFromEx(tp,tp,c)>0 and aux.MustMaterialCheck(c,tp,EFFECT_MUST_BE_XMATERIAL)
end
function cm.ovfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x3905)
end
