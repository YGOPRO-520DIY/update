...
function c81015012.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsReleasable() and Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)~=0
...
function c81015012.spfilter(c,e,tp)
	return c:IsSetCard(0x81a) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		and not Duel.IsExistingMatchingCard(c81015012.bfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,nil,c)
end
function c81015012.bfilter(c,tc)
	return tc:IsCode(c:GetCode()) and (c:IsLocation(LOCATION_HAND) or c:IsLocation(LOCATION_GRAVE))
end
