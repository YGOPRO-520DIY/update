...
function c15000014.filter(c)  
	return c:IsSetCard(0xf30) and (c:IsAbleToHand() or c:IsAbleToGrave()) and not c:IsCode(15000014)
end  
...
function c15000014.operation(e,tp,eg,ep,ev,re,r,rp)  
	...
	if g:GetCount()>0 then  
		local sc=g:GetFirst()  
		if sc then  
			if (not sc:IsAbleToHand() or Duel.SelectOption(tp,1190,1191)==1) then  
